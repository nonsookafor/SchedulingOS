...
{ 
    ...
    int bt_copy[n];
    int finished = 0, time = 0, mini = 1000;
    int smallest = 0, complete_time;
    int check = 0;
      
    for (int i = 0; i < n; i++)
        bt_copy[i] = plist[i].bt;
    ...
    while (finished != n) 
    {
        for (int j = 0; j < n; j++) 
        {
            if ((plist[j].art <= time) && (bt_copy[j] < mini) && bt_copy[j] > 0) 
            {
                mini = bt_copy[j];
                smallest = j;
                check = 1;
        ...
        mini = bt_copy[smallest];
        if (mini == 0)
            mini = 1000;
    

        if (bt_copy[smallest] == 0) 
        {
            finished++;
            check = 0;
            complete_time = time + 1;
    
            plist[smallest].wt = complete_time - plist[smallest].bt - plist[smallest].art;
    
            if (plist[smallest].wt < 0)
                plist[smallest].wt = 0;
        }
        time++;
    }
...
{ 
     ...
     */ 
		...
		process1 = (ProcessType*)this;
		ProcessType *process2;
		process2 = (ProcessType*)that;
